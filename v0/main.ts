...
import { isAppleTouchPlatform, isTabletWebKitTouchDevice } from "./src/utils/deviceUtils";
...
class NativePdfAnnotatorSession {
	...
	private syncPdfPageTemplateBackground(surface: PageSurface, width: number, height: number): void {
		...
		const renderKey = [
			...
			pageTemplate.paperColor,
			JSON.stringify(pageTemplate.pageSize),
			Math.round(width * 100) / 100,
			Math.round(height * 100) / 100
		].join(":");
		if (backgroundEl.dataset.renderKey !== renderKey) {
			this.applyRenderedTemplateBackground(backgroundEl, width, height, templatePage);
			backgroundEl.dataset.renderKey = renderKey;
		}
	...
	private resumeCommittedRenderingAfterInkInput(): void {
		...
		this.redrawHandle = window.requestAnimationFrame(() => {
			this.redrawHandle = null;
			if (this.hasActiveTransientRender() || this.isFingerPanRenderingPaused()) {
				return;
			}
			this.flushDeferredScrollRedraws();
		});
	...
	private pauseCommittedRenderingForFingerPan(): void {
		for (const pageNumber of this.pageRenderJobs.keys()) {
			this.pendingRedrawPages.add(pageNumber);
		}
		for (const pageNumber of this.pageRenderPublications.keys()) {
	...
	private resumeCommittedRenderingAfterFingerPan(): void {
		if (this.isFingerPanRenderingPaused() || !this.annotationDocument || this.redrawHandle !== null) {
			return;
		}
		this.redrawHandle = window.requestAnimationFrame(() => {
			this.redrawHandle = null;
			if (this.isFingerPanRenderingPaused() || this.hasActiveTransientRender()) {
				return;
			}
			this.forceRedrawVisibleAnnotations();
		});
	...
	private setOverlayCursor(surface: PageSurface, cursor: string, cursorTool = this.currentTool): void {
	...
	private readonly handleDocumentTouchStart = (event: TouchEvent): void => {
		...
		const matchingTouch = pendingPointer
			...
			: null;
		if (!pendingPointer || !matchingTouch || !this.annotationMode || this.getInkInputPolicy() !== "pen-mouse-only" || !isAppleTouchPlatform()) {
			this.clearPendingWebKitTouchPointer();
			return;
		}
	...
	private handleFingerPanPointerDown(event: PointerEvent): boolean {
		...
		this.pauseCommittedRenderingForFingerPan();
		try {
