...
assertContains("main.ts", mainTs, "if (next !== pageEl) { pageContainer.insertBefore(pageEl, next); }", "synthetic pages must retain stable DOM order after their anchor during layout refreshes");
...
assertContains("src/stores/annotationStore.ts migrateForRename", renameMigrationSource, "sourceFile: identity.path", "PDF rename must update the sidecar source path");
assertContains("src/stores/annotationStore.ts migrateForRename", renameMigrationSource, "const identity = getPdfIdentity(file)", "PDF rename must refresh the sidecar PDF identity");
assertNotContains("src/stores/annotationStore.ts migrateForRename", renameMigrationSource, "adapter.remove(oldSidecarPath)", "PDF rename must preserve the old sidecar for recovery");
...
assertContains("src/export/mixedDocumentExport.ts", mixedExportTs, "renderMixedPagesToPdfBytes(app, sourceFile, annotationDocument, mixedEntries, realPdfPageCount, false)", "native working PDF should keep annotations editable in sidecar");
