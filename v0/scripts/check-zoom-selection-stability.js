...
assertContains(
	"main.ts",
	mainTs,
	"backgroundEl.dataset.renderKey",
	"same-size background restoration must reuse its rendered template image"
);
assertContains(
	"main.ts",
	mainTs,
...
const scheduledZoomRestoreFrames = scheduleZoomAnchorRestore.split("window.requestAnimationFrame(").length - 1;
...
assertContains(
	...
	"page resize observation must correct pinch scroll drift before the resized page is painted"
);
assertContains(
	"main.ts",
	mainTs,
