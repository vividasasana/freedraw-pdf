...
function assertInsideProject(targetPath) {
	...
	if (relativePath === "" || relativePath.startsWith("..") || path.isAbsolute(relativePath)) {
		throw new Error(`Refusing to write outside the v1 project: ${targetPath}`);
	}
}
...
function buildSamplePdf() {
	const pageOneStream = [
		...
		"BT /F1 24 Tf 72 710 Td (Freedraw PDF v1 test - page 1) Tj ET",
	...
	const pageTwoStream = [
		...
		"BT /F1 24 Tf 72 710 Td (Freedraw PDF v1 test - page 2) Tj ET",
...
function verifyVault() {
	...
	if (installedManifest.id !== manifest.id || installedManifest.version !== manifest.version) {
		throw new Error("Installed test-vault manifest does not match the v1 release manifest.");
	}
	...
	if (!installedMain.equals(releaseMain)) {
		throw new Error("Installed test-vault main.js does not match the v1 build.");
	}
...
writeText(
	path.join(vaultRoot, "Start Here.md"),
	`# Freedraw PDF v1 test vault

This vault contains the current **v1** build. The original project outside \`v1\` is not used by this vault.

1. Open this folder as a vault in Obsidian.
2. If Obsidian asks, trust the vault and enable **Freedraw PDF** under **Settings → Community plugins**.
3. Open [[Test PDF.pdf]], choose **Annotate**, and test with Apple Pencil, one-finger pan, and two-finger pinch zoom.
...
- Existing strokes appear immediately and touch-erased strokes round off without delayed redraws.
...
The vault keeps your test annotations and plugin settings when the package command is run again; only the three plugin release files are refreshed.
`,
	{ preserveExisting: true }
);
