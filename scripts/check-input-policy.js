...
assertContains("src/config.ts", configTs, "inkInputPolicy: \"pen-mouse-only\"", "default policy must reserve finger input for navigation");
...
assertContains("main.ts", mainTs, "pointerEvents: enabled ? \"auto\" : \"none\"", "the annotation surface must own single-contact input before WebKit can claim Pencil as a pan");
assertContains("main.ts", mainTs, "this.fingerPanFrameHandle = this.ownerWindow.requestAnimationFrame", "finger panning must batch scroll writes to animation frames");
...
assertContains("main.ts", mainTs, "this.currentStroke && canvas && (event.pointerType === \"touch\" || event.pointerType === \"pen\")", "touch pointer-up must preserve very short strokes");
