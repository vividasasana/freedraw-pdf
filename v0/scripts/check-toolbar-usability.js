...
assertContains("main.ts", mainTs, "window.visualViewport?.addEventListener(\"resize\", this.handleVisualViewportChange", "software-keyboard viewport changes must reposition plugin UI");
...
assertContains("main.ts", mainTs, "const visualViewport = window.visualViewport;", "popup placement must use the visual viewport on iPad");
...
assertContains("main.ts", mainTs, ".setChecked(this.currentTool === shape.tool)", "shape menu must identify the active shape");
