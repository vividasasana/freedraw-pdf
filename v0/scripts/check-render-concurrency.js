...
async function main() {
	...
	assert(pageRenderJobSource.includes("this.isFingerPanRenderingPaused()"), "committed page rendering must not start during finger pan or inertia");
	assert(pageRenderJobSource.includes("this.pendingRedrawPages.add(pageNumber);"), "pages skipped during finger pan must remain queued for refresh");
	...
	assert(fingerPanDownSource.includes("this.pauseCommittedRenderingForFingerPan();"), "finger pan must cancel in-flight committed rendering before scrolling");
