...
// GoodNotes-like ink should settle the hand without visibly trailing the tip.
// Keep the live window shorter than the committed window so the preview remains
// responsive while the final stroke still benefits from a little more cleanup.
const INK_SMOOTHING_WINDOW_MS = 28;
const INK_LIVE_SMOOTHING_WINDOW_MS = 18;
...
// Point coordinates are normalized to the page, so these values are normalized
// page-widths per second rather than CSS pixels per second. The old thresholds
// were above normal handwriting speed and consequently smoothed almost every
// segment as if it were a slow corner.
const INK_SPEED_FLOOR = 0.12;
const INK_SPEED_CEILING = 0.7;
...
// Native Pencil pressure should remain recognizable. Smoothing the signal is
// preferable to amplifying it, which makes light strokes unexpectedly heavy.
const INK_PRESSURE_GAIN = 1;
const INK_PRESSURE_SLEW_PER_WIDTH = 0.24;
const INK_PRESSURE_SMOOTHING_ALPHA = 0.32;
...
export function appendStrokePoints<TPoint extends InkPoint, TStroke extends InkStroke & { points: TPoint[] }>(
	...
	for (const sample of points) {
		...
		const pressure = previousPoint
			...
			: clamp(sample.pressure * INK_PRESSURE_GAIN, 0.06, 1);
...
function getTimedWindowAverage(points: InkPoint[], index: number, windowMs = INK_SMOOTHING_WINDOW_MS): InkPoint {
	const point = points[index];
	...
	for (let scan = index; scan >= 0; scan -= 1) {
		...
		if (typeof candidate.t !== "number" || point.t - candidate.t > windowMs) {
			break;
		}
		x += candidate.x;
		y += candidate.y;
	...
	for (let scan = index + 1; scan < points.length; scan += 1) {
		const candidate = points[scan];
		if (typeof candidate.t !== "number" || candidate.t - point.t > windowMs) {
			break;
		}
		x += candidate.x;
		y += candidate.y;
...
function getSmoothedStrokePoints(
	points: InkPoint[],
	windowMs = INK_SMOOTHING_WINDOW_MS,
	maximumWeight = 0.28
): InkPoint[] {
	if (points.length < 3) {
		return points;
	...
	return points.map((point, index) => {
		...
		const windowAverage = getTimedWindowAverage(points, index, windowMs);
		if (windowAverage !== point) {
			...
			const smoothingWeight = clamp(maximumWeight * (1 - speedBlend) * cornerFactor, 0.04, maximumWeight);
			return {
...
function getRenderStrokePoints(
	points: InkPoint[],
	predictTail: boolean,
	predictionStrength = 1,
	renderMode: "live" | "committed" = predictTail ? "live" : "committed"
): InkPoint[] {
	...
	// A live call without prediction is the release preview. It must use the
	// same geometry as the committed pass or the stroke will snap on pointer-up.
	const predictedLive = renderMode === "live" && predictTail && (predictionStrength ?? 1) > 0;
	const smoothingWindow = predictedLive ? INK_LIVE_SMOOTHING_WINDOW_MS : INK_SMOOTHING_WINDOW_MS;
	const stabilizationWeight = predictedLive
		? 0.18 + (settings.streamline * 0.08)
		: 0.22 + (settings.streamline * 0.1);
	const preparedPoints = getSmoothedStrokePoints(
		resampleStrokePoints(inputPoints),
		smoothingWindow,
		stabilizationWeight
	);
...
export function getSmoothInkStrokeOutline(
	...
	try {
		...
		void renderMode;
		const renderPoints = getRenderStrokePoints(points, predictTail, options.predictionStrength, renderMode);
...
function drawUniformSmoothStroke(
	...
	void renderMode;
	const renderPoints = getRenderStrokePoints(points, predictTail, predictionStrength, renderMode);
